...
def check_gemini(text):
    ...
    try:
        ...
        prompt = f"あなたは、以下の「てとぼ」などの単語が含まれる文章が、本当にテトリスの対戦相手を募集する意図があるのかどうかを判定しなさい。「てとぼ」「テトぼ」はテトリス募集の略称であり、15先などは15本先取の略称、3000↑などは、レート制限を表します。注意例:帰ってとぼとぼ帰宅:「てとぼ」が含まれるが、「て」に「とぼとぼ」がついただけで募集の意図はない。\nまず判断の根拠を述べ、最後に改行して、意図がある場合は1、ない場合は0の数字1文字のみを出力しなさい。\n\n{text}"
